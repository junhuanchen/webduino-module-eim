...
我想了想，我觉得还是用 Python 实现吧，因为我不用重写各种 Python 已经实现的库，也不需要浏览器中对模块的跨平台，本身 Python 的许多实现就已经满足了主流跨平台，从时间上、成本上、结果上考虑，选择借助 Python 的代码库是最自然的想法。